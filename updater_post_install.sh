...
surf_registrations_enabled="$(grep mastodon.surf_registrations_enabled /ebsa/config/services.config | cut -d '=' -f 2 | head -1)"
surf_registration_token="$(grep mastodon.surf_registration_token /ebsa/config/services.config | cut -d '=' -f 2 | head -1)"
...
if [[ "${surf_registrations_enabled}" == "true" ]] ; then
  echo "SURF_REGISTRATIONS_ENABLED=${surf_registrations_enabled}" >> /tmp/env.production
  echo "SURF_REGISTRATION_TOKEN=${surf_registration_token}" >> /tmp/env.production
else
